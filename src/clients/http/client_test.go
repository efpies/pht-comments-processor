...
func TestClientConfigureRequest(t *testing.T) {
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.applyHeaders && (body == nil || *body != expectedBody || sc != 200 || err != nil) ||
				!tt.applyHeaders && (body != nil || sc == 200 || err == nil) {
				t.Errorf("headers not matched (body: %s, expected: %s) (status: %d, expected: %d) (err: %s)", utils.SafeDeref(body), expectedBody, sc, 200, err)
			}
		})
	}
}
